...
class _MinifigDetailsScreenState extends State<MinifigDetailsScreen> {
  ...
  Future<void> _loadDetails() async {
    try {
      final data = await _apiService.getMinifigDetails(
        widget.minifigure.figNum,
      );
  ...
  void _addToCollection() async {
    final res = await _apiService.addMinifigToCollection(
      widget.minifigure.figNum,
    );
    if (mounted) {
                  ...
                  Row(
                    children: [
                      ...
                      Chip(
                        ...
                        label: Text(
                          '${widget.minifigure.numParts} piezas',
                          style: const TextStyle(color: Colors.white),
                        ),
                        backgroundColor: const Color(0xFF2D2D2D),
                        side: BorderSide.none,
            ...
            _isLoading
                ...
                ? const Center(
                    child: Padding(
                      ...
                      child: Text(
                        'Esta minifigura no aparece en ningún set registrado.',
                        ...
                        child: ListTile(
                          ...
                          onTap: () async {
                            ...
                            try {
                              ...
                              final result = await _apiService.getAllSets(
                                search: set.setNum,
                              );
                              ...
                              LegoSet fullSet = set;
                              if (result['sets'] != null &&
                                  (result['sets'] as List).isNotEmpty) {
                              ...
                              Navigator.push(
                                ...
                                MaterialPageRoute(
                                  builder: (context) =>
                                      SetDetailsScreen(legoSet: fullSet),
                                ),
                              );
                            } catch (e) {
                              Navigator.pop(context); // Quita el loading
                              // Navegamos con el set incompleto si falla internet
                              Navigator.push(
                                context,
                                MaterialPageRoute(
                                  builder: (context) =>
                                      SetDetailsScreen(legoSet: set),
                                ),
                              );
                            }
                          },
