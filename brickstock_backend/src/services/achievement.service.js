    ...
    try {
        ...
        if (count === 0) {
            const initialAchievements = [
                ...
                { code: 'VALUE_5000', name: 'Magnate del ABS', description: '5.000€ en LEGO. Tu colección vale su peso en oro (o en plástico).', icon: 'account_balance', conditionType: 'TOTAL_VALUE', conditionValue: 5000 },
...
const syncCollectionAchievements = async (userId) => {
    try {
        ...
        const themeAgg = await Collection.aggregate([
            { $match: { userId: userObjId, "marketDataCache.themeId": { $ne: null } } },
            { $group: { _id: "$marketDataCache.themeId", count: { $sum: "$quantity" } } },
            { $sort: { count: -1 } },
            { $limit: 1 }
        ]);
